...
- загрузить CSV‑файл через веб‑интерфейс;
- автоматически определить типы колонок;
- сохранить данные в PostgreSQL в виде отдельной таблицы;
- построить набор типовых визуализаций:
	- BarChart - date(x)+value(y)+period(tablename)
	- BarChart - category(x)+value(y)+period(tablename)
	- Piechart - category(x)+value(y)+period(tablename)

Аутентификация и роли намеренно отсутствуют — проект минималистичный, фокус на быстрой загрузке и исследовании данных. Архитектура рассчитана на один backend‑сервис и один frontend‑сервис, развёрнутые рядом с одной БД PostgreSQL.

Асинхронная обработка загрузок реализована через внутренний пул потоков Spring Boot и таблица `upload_jobs` для отслеживания статуса задач.

Глобальное состояние покрывается React Query + локальным состоянием / контекстом.
